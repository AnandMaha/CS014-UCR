...
void BSTree::insert(const string& str) {
    //if tree is empty, set root with value str, and dupNum to 1
    if(root == nullptr) {
        root = new Node(str, 1);
        return;
    }
    Node* curr = root;
    while(curr != nullptr) {
        //3 cases: curr->str == str, curr->str < str, else curr->str > str
        //return whenever insertion is complete
        if(curr->str == str) {
            curr->dupNum++;
            return;
        }
        else if(curr->str < str) {
            //check if curr->rghtChld is nullptr, and if so, insert
